...
# Rename cell subtypes from meta
#setDT(sc1meta)
#sc1meta[Cell_subtype == "Prolif Tcell", Cell_subtype := "Prolif T"]
#sc1meta[Cell_subtype == "Memory GC-indep", Cell_subtype := "GC B"]
#sc1meta[Cell_subtype == "Immunosuppr Mac", Cell_subtype := "Suppr Mac"]
#sc1meta[Cell_subtype == "Cap", Cell_subtype := "Capillary"]
#sc1meta[Cell_subtype == "Tip cells", Cell_subtype := "Tip"]
#sc1meta[Cell_subtype == "IFN", Cell_subtype := "IFN EC"]
#sc1meta[Cell_subtype == "Stalk cells", Cell_subtype := "Stalk"]
#
## Same for conf
...
plot_gene_signature =  function(inpConf, inpMeta, inp, inpGrp, inpPlt,
                                inpsub1, inpsub2,inpsub3, inpsub4, inpH5, inpGene,
                                inpcols, inpfsz, inpcol,inpsiz,inplab, tab, save = FALSE){
  # Hardcoded for troubleshooting purposes
  # #TODO remove this
  # inpConf = sc1conf
  # inpMeta = sc1meta
  # inp = paste0(msigdbr_list[["HALLMARK_INFLAMMATORY_RESPONSE"]],collapse = "\n")
  # inpGrp = "Cancer_type_early_adv"
  # inpPlt = "?"
  # inpsub1 = "Major_cell_type"
  # inpsub2 = NULL
  # inpsub3 = sc1def$grp2
  # inpsub4 = NULL
  # inpH5 = "sc1gexpr.h5"
  # inpGene = sc1gene
  # inpcols = "Black-Violet-Yellow"
  # inpfsz = "Small"
  # inpcol = "Black-Violet-Yellow"
  # inpsiz = 2
  # inplab = TRUE
  # tab = "correlation"
  # save = FALSE
  ...
  #if(!exists("assay.data")) assay.data = readRDS("assay_data.rds")

  # Transform DFs to DTs
  setDT(df_prop)
  setDT(major_types)
  setDT(SampleID_TumorType)
  setDT(barcodes)

  # Rename certain columns of df_prop #TODO:
  #setnames(df_prop,
           #c("Prolif Tcell", "Memory GC-indep", "Immunosuppr Mac", "Cap", "Tip cells", "IFN", "Stalk cells"),
           #c("Prolif T", "GC B", "Suppr Mac", "Capillary", "Tip", "IFN EC", "Stalk"))

  #Set seed and default values
  set.seed(1)
  ...
  for (j in 1:length(x = features)) {
    ctrl.use[[1]] =  c(ctrl.use[[1]],
                       names(x = sample(x = data.cut[which(x = data.cut == data.cut[features[j]])],
                                        size = ctrl,
                                        replace = FALSE)))
  }
  ...
  start_time = Sys.time()
  ##I had to split due to the size
  features.scores.use = lapply(1:10, function(i) {
    print(paste0("Iteration number", i))
    assay.data = readRDS(paste0("assay_data_", i, ".rds"))
    ...
    features.use =  features.use[features.use %in% row.names(assay.data)]
    ...
    features.score.use = data.table(Gene_signature = as.numeric(t(features.scores.use)),
                                    sampleID = colnames(assay.data))
    return(features.score.use)
  }) |> rbindlist()
  end_time =  Sys.time()
  time_diff =  round(difftime(end_time, start_time, units = "secs"))
  print(paste("Time taken: :", time_diff, "seconds"))

  #Now merge with metadata
  sig1 = merge(features.scores.use, sc1meta, by = "sampleID")
  sig1 = merge(sig1, barcodes, by.x = "sampleID", by.y = "barcodes")
  ...
  sig1[, plot := TRUE]
  if(!is.null(inpsub2)){
    sig1[, plot := sig1[[inpsub1]] %in% inpsub2]
    bgCells = TRUE
  }
  if(!is.null(inpsub4)) {
    bgCells = TRUE
    sig1[, plot := sig1$plot == TRUE & sig1[[inpsub3]] %in% inpsub4]
  }

  ## Use only the selected ones
  df_sign = sig1[plot == TRUE, .(Gene_signature = mean(Gene_signature), ncells = .N), by = SampleID]
  df_sign = df_sign[ncells >= 20]

  # Just filter samples to make sure dataframes are the same size
  df_sign =  df_sign[SampleID %in% df_prop$SampleID,]
  ...
  if(tab == "correlation"){
    # New function to calculate correlation
    calc_cor = function(df) {
      a = rcorr(as.matrix(df), type = "spearman")
      r = reshape2::melt(a$r)
      p = reshape2::melt(a$P)
      setDT(r)
      setDT(p)
      dt = merge(r, p, by = c("Var1", "Var2"))
      dt[, method := "spearman"]
      setnames(dt, c("var1", "var2", "cor", "p", "method"))
    }

    start_time = Sys.time()
    # Merge with df_prop
    df_orig = merge(df_sign, df_prop, by = "SampleID")
    df_copy = copy(df_orig)
    ...
    df = if(filter_samples) df_orig[ncells_noLQ_noD_obj_TC >= 20] else df_orig

    df_cols = c(subtypes$TC_types, "Gene_signature")
    cor_TC = calc_cor(df[, ..df_cols])[var2 == "Gene_signature" & var1 != "Gene_signature"]

    # BC
    # Keep only the samples with 10 or more BCs
    df = if (filter_samples) df_orig[ncells_noLQ_noD_obj_BC >= 10] else df_orig

    df_cols = c(subtypes$BC_types, "Gene_signature")
    cor_BC = calc_cor(df[, ..df_cols])[var2 == "Gene_signature" & var1 != "Gene_signature"]

    # MM
    # Keep only the samples with 20 or more MMs
    df = if (filter_samples) df_orig[ncells_noLQ_noD_obj_MM >= 20] else df_orig

    df_cols = c(subtypes$MM_types, "Gene_signature")
    cor_MM = calc_cor(df[, ..df_cols])[var2 == "Gene_signature" & var1 != "Gene_signature"]

    # DC
    # Keep only the samples with 10 or more DCs
    df = if (filter_samples) df_orig[ncells_noLQ_noD_obj_DC3 >= 10] else df_orig

    df_cols = c(subtypes$DC3_types, "Gene_signature")
    cor_DC3 = calc_cor(df[, ..df_cols])[var2 == "Gene_signature" & var1 != "Gene_signature"]

    # EC
    # Keep only the samples with 10 or more ECs
    df = if (filter_samples) df_orig[ncells_noLQ_noD_obj_EC >= 10] else df_orig

    df_cols = c(subtypes$EC_types, "Gene_signature")
    cor_EC = calc_cor(df[, ..df_cols])[var2 == "Gene_signature" & var1 != "Gene_signature"]
    ...
    cor_subtypes = rbindlist(list(cor_TC, cor_BC, cor_MM, cor_DC3, cor_EC))
    setnames(cor_subtypes, c("subtype", "var2", "Score", "Pval", "name"))
    cor_df = merge(cor_subtypes[, .(subtype, Score, Pval, name)], major_types, by = "subtype")
    ...
    setorder(cor_df, -Score)
    cor_df[, subtype := factor(subtype, levels = unique(subtype))]
    cor_df[, major_type := factor(major_type, levels = c("NK/T cells", "B cells",  "Monocytes and macrophages", "Dendritic cells", "Endothelial cells"))]
    ...
    print("pancancer done")
    end_time =  Sys.time()
    print(paste("Time for pancancer plot:", end_time - start_time, "seconds"))
    ...
    selected_cancertypes = as.character(unique(sig1$Cancer_type_early_adv[sig1$plot == T]))
    res = parallel::mclapply(selected_cancertypes, function(cancer_type) {
      # for (cancer_type in unique(SampleID_TumorType$TumorType3)) {


      # Keep only the samples of the selected tumor type
      samples = SampleID_TumorType[TumorType3 == cancer_type, SampleID]
      df_orig_cancerType = df_orig[SampleID %in% samples]

      ###This line was added because if you have only one 1 sample the correlation gives an error
      if(nrow(df_orig_cancerType) > 1){
        # TC
        df = if (filter_samples) df_orig_cancerType[ncells_noLQ_noD_obj_TC >= 20] else df_orig_cancerType
        ...
        if (nrow(df) > 4) {
          ...
          cor_TC = data.table(var1 = subtypes$TC_types,
                              var2 = "Gene_signature",
        ...
        rm(df)
        # BC
        df = if (filter_samples) df_orig_cancerType[ncells_noLQ_noD_obj_BC >= 20] else df_orig_cancerType
        ...
        if (nrow(df) > 4) {
          ...
          cor_BC = data.table(var1 = subtypes$BC_types,
                              var2 = "Gene_signature",
        ...
        rm(df)
        # EC
        df = if (filter_samples) df_orig_cancerType[ncells_noLQ_noD_obj_EC >= 20] else df_orig_cancerType
        ...
        if (nrow(df) > 4) {
          ...
          cor_EC = data.table(var1 = subtypes$EC_types,
                              var2 = "Gene_signature",
        ...
        rm(df)
        # MM
        df = if (filter_samples) df_orig_cancerType[ncells_noLQ_noD_obj_MM >= 20] else df_orig_cancerType
        ...
        if (nrow(df) > 4) {
          ...
          cor_MM = data.table(var1 = subtypes$MM_types,
                              var2 = "Gene_signature",
        ...
        rm(df)
        # DC
        df = if (filter_samples) df_orig_cancerType[ncells_noLQ_noD_obj_DC3 >= 20] else df_orig_cancerType
        ...
        if (nrow(df) > 4) {
          ...
          cor_DC3 = data.table(var1 = subtypes$DC3_types,
                              var2 = "Gene_signature",
    ...
    res[, subtype := factor(subtype, levels = levels(cor_df$subtype))]
    res[, major_type := factor(major_type, levels = c("NK/T cells", "B cells",  "Monocytes and macrophages", "Dendritic cells", "Endothelial cells"))]
    res[, TumorType3 := factor(TumorType3, levels = rev(c("BC_early", "BC_adv", "CC", "CRC", "GBM", "HCC", "HGSOC", "HNSCC", "MEL", "NSCLC_early", "NSCLC_adv")))]
    ...
    # Add dendrogram
    df = p$data[, .(subtype, Score, TumorType3)]
    df = as.tibble(dcast(df, TumorType3~subtype, value.var = "Score", fill = 0)) %>% column_to_rownames("TumorType3")
...
scVioBox =  function(inpConf, inpMeta, inp1, inp2,
  ...
  for(i in 1:length(inp2)){
    inp2[i] -> genes2plot
    # Prepare ggData
    ggData = inpMeta[, c(inpConf[UI == inp1]$ID, inpConf[UI == inpsub1]$ID,inpConf[UI == inpsub3]$ID),
                     with = FALSE]
    colnames(ggData) = c("X", "sub","sub2")
    ...
    if(length(inpsub4) != 0 & length(inpsub4) != nlevels(ggData$sub2)){
      ggData = ggData[sub2 %in% inpsub4]
    }
...
scProp =  function(inpConf, inpMeta, inp1, inp2, inpsub1, inpsub2, inpsub3, inpsub4,
                   inptyp, inpflp, inpfsz){
  if(is.null(inpsub1)){inpsub1 = inpConf$UI[1]}
  # Prepare ggData
  ggData = inpMeta[, c(inpConf[UI == inp1]$ID, inpConf[UI == inp2]$ID,
                       inpConf[UI == inpsub1]$ID, inpConf[UI == inpsub3]$ID),
                   with = FALSE]
  colnames(ggData) = c("X", "grp", "sub","sub2")
...
scBubbHeat <- function(inpConf, inpMeta, inp, inpGrp, inpPlt,
                       inpsub1, inpsub2, inpsub3, inpsub4, inpH5, inpGene, inpScl, inpRow, inpCol,
  ...
  # Prepare ggData (added inpsub3, sub2 etc.)
  h5file <- H5File$new(inpH5, mode = "r")
  h5data <- h5file[["grp"]][["data"]]
  ggData = data.table()
  for(iGene in geneList$gene){
    tmp = inpMeta[, c("sampleID", inpConf[UI == inpsub1]$ID, inpConf[UI == inpsub3]$ID), with = FALSE]
    colnames(tmp) = c("sampleID", "sub", "sub2")
...
# Plot gene expression bubbleplot / heatmap (1 gene 2 variables) ----
scBubbHeat_1g2v <- function(inpConf, inpMeta, inp, inpGrp1, inpGrp2, inpPlt,
                            inpsub1, inpsub2, inpsub3, inpsub4, inpH5, inpGene, inpScl, inpRow, inpCol,
                            inpcols, inpfsz, save = FALSE){
  ...
  tmp = inpMeta[, c("sampleID", inpConf[UI == inpsub1]$ID, inpConf[UI == inpsub3]$ID), with = FALSE]
  colnames(tmp) = c("sampleID", "sub", "sub2")
  tmp$grpBy1 = inpMeta[[inpConf[UI == inpGrp1]$ID]]
  tmp$grpBy2 = inpMeta[[inpConf[UI == inpGrp2]$ID]]
  tmp$geneName = iGene
  tmp$val = h5data$read(args = list(inpGene[iGene], quote(expr=)))
  ggData = rbindlist(list(ggData, tmp))
  ...
  # To fix (>1 or >=1, zero not detected > plots everything)
  shiny::validate(need(uniqueN(ggData$grpBy1) > 1, "Only 1 group present, unable to plot!"))
  shiny::validate(need(uniqueN(ggData$grpBy2) > 1, "Only 1 group present, unable to plot!"))

  # doesnt work because ggData is unfiltered since inpsub2=0
  # shiny::validate(need(!is.null(ggData), "No groups present, unable to plot!"))
  ...
  colRange = range(ggData$val)
  if(inpScl){
    ggData[, val:= scale(val), keyby = "grpBy1"]
    colRange = c(-max(abs(range(ggData$val))), max(abs(range(ggData$val))))
  }

  # hclust row/col if necessary
  ggMat = dcast.data.table(ggData, grpBy1~grpBy2, value.var = "val")
  tmp = ggMat$grpBy1
  ggMat = as.matrix(ggMat[, -1])
  rownames(ggMat) = tmp
  if(inpRow){
    ...
    ggRow = ggplot() + coord_flip() +
      geom_segment(data = hcRow$segments, aes(x=x,y=y,xend=xend,yend=yend)) +
      scale_y_continuous(breaks = rep(0, uniqueN(ggData$grpBy2)),
                         labels = unique(ggData$grpBy2), expand = c(0, 0)) +
      scale_x_continuous(breaks = seq_along(hcRow$labels$label),
                         labels = hcRow$labels$label, expand = c(0, 0.5)) +
      sctheme(base_size = sList[inpfsz]) +
      theme(axis.title = element_blank(), axis.line = element_blank(),
            axis.ticks = element_blank(), axis.text.y = element_blank(),
            axis.text.x = element_text(color="white", angle = 45, hjust = 1))
    ggData$grpBy1 = factor(ggData$grpBy1, levels = hcRow$labels$label)
  } else {

    # to fix! OK
    # ggData$grpBy1 = factor(ggData$grpBy1, levels = rev(geneList$gene))
    ggData$grpBy1 = factor(ggData$grpBy1)
  ...
  if(inpCol){
    ...
    ggCol = ggplot() +
      ...
      scale_x_continuous(breaks = seq_along(hcCol$labels$label),
                         labels = hcCol$labels$label, expand = c(0.05, 0)) +
      scale_y_continuous(breaks = rep(0, uniqueN(ggData$grpBy1)),
                         labels = unique(ggData$grpBy1), expand=c(0,0)) +
      sctheme(base_size = sList[inpfsz], Xang = 45, XjusH = 1) +
      theme(axis.title = element_blank(), axis.line = element_blank(),
            axis.ticks = element_blank(), axis.text.x = element_blank(),
            axis.text.y = element_text(color = "white"))
    ggData$grpBy2 = factor(ggData$grpBy2, levels = hcCol$labels$label)
  }
  ...
  if(inpPlt == "Bubbleplot"){
    ...
    # Bubbleplot
    ggOut = ggplot(ggData, aes(grpBy2, grpBy1, color = val, size = prop)) +
      geom_point() +
      sctheme(base_size = sList[inpfsz], Xang = 45, XjusH = 1) +
      scale_x_discrete(expand = c(0.05, 0)) +
      scale_y_discrete(expand = c(0, 0.5)) +
    ...
    # Heatmap
    ggOut = ggplot(ggData, aes(grpBy2, grpBy1, fill = val)) +
      geom_tile() +
      sctheme(base_size = sList[inpfsz], Xang = 45, XjusH = 1) +
      scale_x_discrete(expand = c(0.05, 0)) +
      scale_y_discrete(expand = c(0, 0.5)) +
  ...
  # Final tidy
  ggLeg = g_legend(ggOut)
  ggOut = ggOut + theme(legend.position = "none") + ggtitle(iGene)
  if(!save){
    if(inpRow & inpCol){ggOut =
      grid.arrange(ggOut, ggLeg, ggCol, ggRow, widths = c(7,1), heights = c(1,7,2),
                   layout_matrix = rbind(c(3,NA),c(1,4),c(2,NA)))
...
shinyServer(function(input, output, session) {
  ...
  updateSelectizeInput(session, "sc1a3inp1", choices = names(sc1gene), server = TRUE,
                       selected = c("CD4","CD8A","ZEB1","CDH1"), options = list(
                         maxOptions = 300, create = TRUE, persist = TRUE, render = I(optCrt)))
  ...
  output$sc1a1oup1.pdf =  downloadHandler(
    filename = function() { paste0("All_",input$sc1a1drX,"_",input$sc1a1drY,"_",
                                   input$sc1a1inp1,".pdf") },
  ...
  output$sc1a2oup1.pdf =  downloadHandler(
    filename = function() { paste0("All_",input$sc1a2drX,"_",input$sc1a2drY,"_",
                                   input$sc1a2inp1,".pdf") },
    content = function(file) { ggsave(
  ...
  output$sc1a3oup1.pdf =  downloadHandler(
    filename = function() { paste0("All_",input$sc1a3drX,"_",input$sc1a3drY,"_",
  ...
  output$sc1b2oup1.pdf =  downloadHandler(
    filename = function() { paste0("All_",input$sc1b2drX,"_",input$sc1b2drY,"_",
  ...
  output$sc1c1oup =  renderPlot({
    ...
    scVioBox(sc1conf, sc1meta, isolate(input$sc1c1inp1), isolate(input$sc1c1inp2),
             isolate(input$sc1c1sub1), isolate(input$sc1c1sub2),isolate(input$sc1c1sub3), isolate(input$sc1c1sub4),
  ...
  output$sc1c1oup.pdf =  downloadHandler(
    filename = function() { paste0("All_",input$sc1c1typ,"_",input$sc1c1inp1,"_",
    ...
    content = function(file) { ggsave(
      ...
      plot = scVioBox(sc1conf, sc1meta, input$sc1c1inp1, isolate(input$sc1c1inp2),
                      input$sc1c1sub1, input$sc1c1sub2, input$sc1c1sub3, input$sc1c1sub4,
  ...
  output$sc1c2oup =  renderPlot({
    scProp(sc1conf, sc1meta, input$sc1c2inp1, input$sc1c2inp2,
           input$sc1c2sub1, input$sc1c2sub2,input$sc1c2sub3, input$sc1c2sub4,
  ...
  output$sc1c2oup.pdf =  downloadHandler(
    filename = function() { paste0("All_",input$sc1c2typ,"_",input$sc1c2inp1,"_",
                                   input$sc1c2inp2,".pdf") },
    content = function(file) { ggsave(
  ...
  output$sc1d1oup =  renderPlot({
    ...
    plot_gene_signature(sc1conf, sc1meta, isolate(input$sc1d1inp), isolate(input$sc1d1grp), isolate(input$sc1d1plt),
                        isolate(input$sc1d1sub1), isolate(input$sc1d1sub2), isolate(input$sc1d1sub3), isolate(input$sc1d1sub4),
                        "sc1gexpr.h5", sc1gene,
  ...
  output$sc1d1oup.pdf =  downloadHandler(
    filename = function() { paste0("All_",input$sc1d1plt,"_",input$sc1d1grp,".pdf") },
    content = function(file) { ggsave(
      file, device = "pdf", height = input$sc1d1oup.h, width = input$sc1d1oup.w ) #TODO: Fix this to plot only the last plot from this function
    })
  output$sc1d1oup.png =  downloadHandler(
    filename = function() { paste0("All_",input$sc1d1plt,"_",input$sc1d1grp,".png") },
    content = function(file) { ggsave(
      file, device = "png", height = input$sc1d1oup.h, width = input$sc1d1oup.w ) #TODO: Fix this to plot only the last plot from this function
    })
  ...
  output$sc1d2oup =  renderPlot({
    ...
    plot_gene_signature(sc1conf, sc1meta, isolate(input$sc1d2inp), isolate(input$sc1d2grp), isolate(input$sc1d2plt),
                        isolate(input$sc1d2sub1), isolate(input$sc1d2sub2),isolate(input$sc1d2sub3), isolate(input$sc1d2sub4),
                        "sc1gexpr.h5", sc1gene,
  ...
  output$sc1d2oup.pdf =  downloadHandler(
    ...
    content = function(file) { ggsave(
      file, device = "pdf", height = input$sc1d2oup.h, width = input$sc1d2oup.w ) #TODO: Fix this to plot only the last plot from this function
    })
  output$sc1d2oup.png =  downloadHandler(
    filename = function() { paste0("Correlation_plot.png") },
    content = function(file) { ggsave(
      file, device = "png", height = input$sc1d2oup.h, width = input$sc1d2oup.w ) #TODO: Fix this to plot only the last plot from this function
    })

  output$sc1bb1oup <- renderPlot({
  req(input$goButton_bb1)
  scBubbHeat(sc1conf, sc1meta, isolate(input$sc1bb1inp), isolate(input$sc1bb1grp), isolate(input$sc1bb1plt),
             isolate(input$sc1bb1sub1), isolate(input$sc1bb1sub2), isolate(input$sc1bb1sub3), isolate(input$sc1bb1sub4),
             "sc1gexpr.h5", sc1gene,
             isolate(input$sc1bb1scl), isolate(input$sc1bb1row), isolate(input$sc1bb1col),
             isolate(input$sc1bb1cols), isolate(input$sc1bb1fsz))
  })

  output$sc1bb1oup.ui <- renderUI({
    withLoader(plotOutput("sc1bb1oup", height = pList3[input$sc1bb1psz]), type = "html", loader = "dnaspin")
  })
  output$sc1bb2oup <- renderPlot({
    req(input$goButton_bb2)
    scBubbHeat_1g2v(sc1conf, sc1meta, isolate(input$sc1bb2inp), isolate(input$sc1bb2grp1), isolate(input$sc1bb2grp2), isolate(input$sc1bb2plt),
                    isolate(input$sc1bb2sub1), isolate(input$sc1bb2sub2), isolate(input$sc1bb2sub3), isolate(input$sc1bb2sub4), "sc1gexpr.h5", sc1gene,
                    isolate(input$sc1bb2scl), isolate(input$sc1bb2row), isolate(input$sc1bb2col),
                    isolate(input$sc1bb2cols), isolate(input$sc1bb2fsz))
    })

    output$sc1bb2oup.ui <- renderUI({
      withLoader(plotOutput("sc1bb2oup", height = pList3[input$sc1bb2psz]), type = "html", loader = "dnaspin")
    })
})
