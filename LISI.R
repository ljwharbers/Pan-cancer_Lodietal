...
DimPlot(object, group.by = Minorcelltype_annotation, pt.size = 0.1, label.size = 5, raster = FALSE, repel = TRUE)
DimPlot(object, group.by = Minorcelltype_annotation, label = TRUE, label.size = 5, pt.size = 0.1, raster = FALSE, repel = TRUE)  + theme_void() + ggtitle("") + theme(legend.position = "none")
DimPlot(object, group.by = Minorcelltype_annotation, label = FALSE, label.size = 5, pt.size = 0.1, raster = FALSE, repel = TRUE) + theme_void() + ggtitle("") + theme(legend.position = "none") 
...
options(repr.plot.widt = 4, height = 2)
ggplot(object@meta.data, aes_string(x= Minorcelltype_annotation, y = "LISI", fill = Minorcelltype_annotation)) + geom_boxplot() + 
